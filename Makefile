...
ifeq ($(OS),Windows_NT)
WAIT_CMD = powershell -Command "while (1 -eq 1) { try { Invoke-WebRequest -UseBasicParsing -Uri $(NEXUS_URL)/service/rest/v1/status -ErrorAction Stop; break } catch { Write-Host 'Nexus not ready, sleeping...'; Start-Sleep -Seconds 5 } }"
else
...
clean: stop
	$(DOCKER_COMPOSE) rm -f
ifeq ($(OS),Windows_NT)
	docker volume rm $$(docker volume ls -qf dangling=true) 2>nul || true
	powershell -Command "if (Test-Path './person-service/build') { Remove-Item -Recurse -Force './person-service/build' }"
else
