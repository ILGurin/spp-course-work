...
/**
 * Описывает основную логику приложения
 * - получение данных по пользователю
 * - регистрирование пользователя (SAGA - оркестрация)
 */
@Slf4j
@Service
        ...
        return personService.register(request) // Mono<UUID> personId
                .flatMap(personId ->
                        keycloakClient.adminLogin()
                                .flatMap(adminToken -> {
                                    var kcUser = new by.cloud.api.dto.KeycloakUserRepresentation(
                                            null,
                                            request.getEmail(),
                                            request.getEmail(),
                                            true,
                                    ...
                                    return keycloakClient.registerUser(adminToken, kcUser)
                                            .flatMap(kcUserId -> {
                                                var cred = new by.cloud.api.dto.KeycloakCredentialsRepresentation(
                                                        "password",
                                                        request.getPassword(),
                                                        false
                                                );
                                            ...
                                            .flatMap(r ->
                                                    keycloakClient.login(
                                                            new by.cloud.keycloak.dto.UserLoginRequest(
                                                                    request.getEmail(),
                                                                    request.getPassword()
                                                            )
                                                    )
