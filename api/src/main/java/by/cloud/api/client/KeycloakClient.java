package by.cloud.api.client;

import by.cloud.api.config.KeycloakProperties;
import by.cloud.api.dto.KeycloakCredentialsRepresentation;
import by.cloud.api.dto.KeycloakUserRepresentation;
import by.cloud.api.exception.ApiException;
import by.cloud.api.util.UserIdExtractor;
import by.cloud.keycloak.dto.TokenRefreshRequest;
import by.cloud.keycloak.dto.TokenResponse;
import by.cloud.keycloak.dto.UserLoginRequest;
...
/**
 * Компонент KeycloakClien необходим для инкапсуляции методов вызова Keycloak:
 * - login пользователя
 * - login админа
 * - обновления токена
